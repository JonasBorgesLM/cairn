...
### There is no committed `CHANGELOG.md`, and that is the decision

Issue #53 asked whether one should exist alongside the GitHub Releases page.
It should not: the API diff above already lands in the release notes,
generated from the real diff at release time, which is what NFR-13 asks for.
A committed file holding the same content would be either a manual copy of
what the release job already produces — a second place for that text to go
stale relative to the first — or itself generated by a step this workflow
would have to grow, for no reader-visible benefit over the releases page a
`go get` or a GitHub visit already surfaces. `git tag -l` plus the releases
